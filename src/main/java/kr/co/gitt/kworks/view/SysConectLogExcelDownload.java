...
@Component("sysConectLogExcelDownload")
public class SysConectLogExcelDownload extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook wb, HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		if (categories.size() > 0) {
			...
			isVO = obj instanceof KwsSysConectLog;
		}

		for(int i = 0; i < categories.size(); i++) {
			KwsSysConectLog category = (KwsSysConectLog) categories.get(i);
			...
			cell.setCellValue(category.getSysNm());
