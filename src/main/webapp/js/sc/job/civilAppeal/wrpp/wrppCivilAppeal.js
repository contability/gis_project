...
wrppCivilAppealObj = {
	...
	CLASS_NAME : "WrppCivilAppealSearch",
	...
	initUi : function() {
		...
		// easyUi datebox init
		$("#searchWrppCivilAppeal .easyui-datebox", that.selector).datebox();
	},
	...
	bindEvents : function() {
		...
		// 초기화
		$(".a_reset_wrppCivilAppealSearch", that.selector).on("click", function() {
			...
			$("select[name=aplHjd]", that.selector).val("");
		...
		// 검색
		$(".a_search_wrppCivilAppealSearch", that.selector).on("click", function() {
			that.search();
	...
	open : function() {
		...
		var url = CONTEXT_PATH + "/civilAppeal/wrpp/searchWrppCivilAppealPage.do";
		var windowOptions = {
			width : 665,
			height : 244,
			top : 120,
	...
	search : function() {
		...
		var url = CONTEXT_PATH + "/civilAppeal/wrpp/listWrppCivilAppeal.do";
		var params = {};
		...
		params.aplHjd = $("select[name=aplHjd]", that.selector).val();
...
// 상수민원 등록
wrppCivilAppealObj.add = {
		
	open : function(ftrIdn) {
		var that = this;
		var layerName = "WTT_WSER_MA";
	...
	save : function(feature) {
		var url = CONTEXT_PATH + "/civilAppeal/wrpp/addWrppCivilAppeal.do";
		var params = {};
...
wrppCivilAppealObj.modify = {
	...
	save : function(fids, props) {
		...
		modifyData.aplHjd = that.data.APL_HJD;
		...
		var url = CONTEXT_PATH + "/civilAppeal/wrpp/" + that.ftrIdn + "/modifyWrppCivilAppeal.do";
		$.post(url, params).done(function() {
			that.ftrIdn = null;
			that.data = null;
			that.originalData = {};
