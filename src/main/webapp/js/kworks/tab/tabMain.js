...
var mainObj = {
	...
	resizeEasyUi : function(){
		$(".div_map_container_tabs").tabs('resize');
		$(".cmmn_tabs").tabs('resize');
...
//		mapToolObj.multiObj.init();			// 다분면 객체 초기화
	...
	createMapTools : function() {
		...
		for(var i=0, len=MAP_TOOLS.length; i < len; i++) {
			...
			if(tool.type == "gisbutton") {
				...
				//if(!tool.id =="multi"|| !tool.id =="roadview"|| !tool.id =="moveAerialMap"){
				if(tool.id !="multi"){
					var id = "a_map_tool_" + tool.id;
					var title = tool.title;
					var format = tool.format?tool.format:"svg";
	...
	bindEvents : function() {
		...
		// 필지 정보
		$('#a_map_tool_spatial_props').click(function(){
			var className = "SearchInfoPrpos";
			that.CLASS_NAME = className;
			var title = "필지정보";
					
			var element = $(this);
			var imageElement = element.find("img");
			if (imageElement.hasClass("node-selected")) {
				...
				selectObj.active(className, "Point", "drawend", function(evt) {
					quickObj.searchPnu(evt.feature, className, title).done(function(pnu) {
						quickObj.useAreaSpatial(evt.feature, className, title).done(function(result) {
							//useAreaObj.open(pnu,result);
							useAreaObj.loadData(pnu, result);
						});
					});
				});
				event.preventDefault();
...
mapToolObj.roadviewObj = {
	...
	init : function() {
		...
		if(window.daum == null || PROJECT_CODE == 'gn') {
	...
	initUi : function(){
		$('#roadview_container').height($('#div_map').height() - $('.heder_snb').height() - $('.tabs-header').height() -2);
		$('#roadview_container').css('top', $('.heder_snb').height());
	},
		...
		try {
			that.interaction = new kworks.interaction.RoadViewDaum({
				...
				executeHandler : function() {
					...
					$("#map_container panel").css("z-index", 7);
					$("#roadview_mode").show();
				}
			});
...
//						}
				...
				if(tool.sysId){
					if(SYS_ID == tool.sysId){
						tagStr += '<a id="'+ id +'" href="#"><img src="'+ CONTEXT_PATH +'/images/kworks/map/tool/'+tool.id+'_on.png" alt="'+ title +'" title="'+ title +'" class="node-selected"/></a>';
					}
				}else{
					tagStr += '<a id="'+ id +'" href="#"><img src="'+ CONTEXT_PATH +'/images/kworks/map/tool/'+tool.id+'_on.png" alt="'+ title +'" title="'+ title +'" class="node-selected"/></a>';
				}
	...
	bindEvents : function() {
		...
		// 토지
		$("#a_quick_menu_prop").click(function() {
			var element = $(this);
			var imageElement = element.find('img');
			
			var node = $('.ul_layer_tree2', that.selector).tree('find', COMMON.PROP);
			
			if(!node.target){
				target = $('#' + node.domId, that.selector);
			}else{
				target = node.target;
			}
			...
			if(imageUtils.isOn(imageElement)){
				...
				$('.ul_layer_tree2', that.selector).tree('uncheck', target);
			}else{
				imageUtils.on(imageElement);
				
				$('.ul_layer_tree2', that.selector).tree('check', target);
			}
		...
		// 건물재산
		$("#a_quick_menu_buid").click(function(){
			var element = $(this);
			var imageElement = element.find('img');
			
			var node = $('.ul_layer_tree2', that.selector).tree('find', COMMON.BUID);
		...
		// 대부내역
		$("#a_quick_menu_loan").click(function(){
			var element = $(this);
			var imageElement = element.find('img');
			
			var node = $('.ul_layer_tree2', that.selector).tree('find', COMMON.LOAN);
		...
		// 무단점유
		$('#a_quick_menu_occp').click(function(){
			var element = $(this);
			var imageElement = element.find('img');
			
			var node = $('.ul_layer_tree2', that.selector).tree('find', COMMON.OCCP);
			
			if(!node.target){
				target = $('#'+ node.domnId, that.selector);
			}else{
				target = node.target;
			}
		...
		/*$('#a_quick_menu_acex').click(function(){
			var element = $(this);
			var imageElement = element.find('img');
			
			var node = $('.ul_layer_tree2', that.selector).tree('find', COMMON.ACEX);
...
//			var title = "정책현황목록 목록";
	...
	useAreaSpatial :  function(feature, className, title) {
		...
		var dataIds = dataObj.getDatabyDataCtgry("CONTINUATION_THEMAMAP");
		
		// 공간 검색
		spatialSearchUtils.search(title, dataIds, filter, function(data) {
...
//				debugger;
	...
	handlerSldChange : function() {
		var land = sldObj.getNamedLayer(COMMON.LAND);
		var buld = sldObj.getNamedLayer(COMMON.BULD);
		var bmlBuidAs = sldObj.getNamedLayer(COMMON.BUID);
		if(land) {
			...
			if(bmlBuidAs && bmlBuidAs.visible){
				imageUtils.on($('#a_quick_menu_buid img'));
			}else{
				imageUtils.off($('#a_quick_menu_buid img'));
			}
		}
	},
