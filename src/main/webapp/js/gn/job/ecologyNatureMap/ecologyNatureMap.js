	...
	isUniqueRow : function(uniqueRows, newRow, fieldInfos) {
		...
		for (var i = 0; i < uniqueRows.length; i++, columnCompareResult = false) {
			...
			for (var j = 0; j < fieldInfos.length; j++) {
				...
				if (fieldInfo.fieldId != 'GEOM' && fieldInfo.fieldId != 'OBJECTID' && fieldInfo.fieldId != 'FTR_IDN'  && fieldInfo.fieldId != 'PNU') {
...
ecologyNatureMapObj.statisticsObj = {
	...
	bindEvents : function(parent) {
		...
		// 생태자연도 등급별 현황
		$(".a_statistics_grade", that.selector).click(function() {
			var url = CONTEXT_PATH + "/clipreport/NEL/ECOLOGY_NATURE_MAP_GRADE/stats.do";			
			popupUtils.open(url, "statistics_grade", { width : 900 , height : 800, left : 300, top:150 });
		});
		
		// 행정구역별 생태특성정보 현황
		$(".a_statistics_administzone", that.selector).click(function() {
			var url = CONTEXT_PATH + "/clipreport/NEL/ECOLOGY_NATURE_MAP_ADMINISTZONE/stats.do";			
			popupUtils.open(url, "statistics_administzone", { width : 900 , height : 800, left : 300, top:150 });
		});

		// 소유자별 생태특성정보 현황
		$(".a_statistics_owner", that.selector).click(function() {
			var url = CONTEXT_PATH + "/clipreport/NEL/ECOLOGY_NATURE_MAP_OWNER/stats.do";			
			popupUtils.open(url, "statistics_owner", { width : 900 , height : 800, left : 300, top:150 });
		});
	}
