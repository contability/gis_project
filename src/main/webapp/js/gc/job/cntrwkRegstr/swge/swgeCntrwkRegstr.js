...
swgeCntrwkRegstrObj = {
	...
	CLASS_NAME : "SwgeCntrwkRegstrSearch",
	...
	initUi : function() {
		...
		// easyUi datebox init
		$("#listSwgeCntrwkRegstr .easyui-datebox", that.selector).datebox();
	},
	...
	bindEvents : function() {
		...
		// 초기화
		$(".a_reset_swgeCntrwkRegstr", that.selector).on("click", function() {
		...
		// 검색
		$(".a_search_swgeCntrwkRegstr", that.selector).on("click", function() {
			that.search();
	...
	open : function() {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/searchSwgeCntrwkRegstrPage.do";
		var windowOptions = {
			width : 655,
			height : 265,
			top : 120,
	...
	search : function() {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/listSwgeCntrwkRegstr.do";
		var params = {};
...
/// 하수 공사대장 조회
swgeCntrwkRegstrObj.view = {
		
	TITLE : "하수공사대장 상세정보",
	
	CLASS_NAME : "SwgeCntrwkRegstrDetail",
	...
	bindEvents : function() {
		...
		// 편집
		$(".btn_modify_swgeCntrwkRegstrDetail", that.selector).on("click", function() {
	...
	open : function(fids) {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/" + that.ftrIdn + "/selectOneSwgeCntrwkRegstrPage.do";
		var windowOptions = {
			width : 680,
			height : 579,
			top : 120,
...
/// 하수 공사대장 수정
swgeCntrwkRegstrObj.modify = {
		
	TITLE : "하수공사대장 수정",
	
	CLASS_NAME : "SwgeCntrwkRegstrModify",
	...
	init : function() {
		...
		$("form", that.selector).ajaxForm({
			...
			success : function(result) {
				if(result) {
					swgeCntrwkRegstrObj.view.open(that.ftrIdn);
					that.close();
					alert("공사대장이 수정되었습니다.");
				}
				else {
	...
	initUi : function() {
		...
		// easyUi datebox init
		$("#swgeCntrwkRegstrModify .easyui-datebox", that.selector).datebox();
	...
	bindEvents : function() {
		...
		// 저장
		$(".btn_save_swgeCntrwkRegstrModify", that.selector).on("click", function() {
			that.modify();
			return false;
		});
	},
	...
	open : function(ftrIdn) {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/" + that.ftrIdn + "/modifySwgeCntrwkRegstrPage.do";
		var windowOptions = {
			width : 680,
			height : 579,
			top : 120,
	...
	modify : function() {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/" + that.ftrIdn + "/modifySwgeCntrwkRegstr.do";
...
/// 하수 공사대장 등록
swgeCntrwkRegstrObj.add = {
		
	TITLE : "하수공사대장 등록",
	
	CLASS_NAME : "SwgeCntrwkRegstrAdd",
	...
	initUi : function() {
		...
		// easyUi datebox init
		$("#swgeCntrwkRegstrAdd .easyui-datebox", that.selector).datebox();
	...
	bindEvents : function() {
		...
		// ehojo 조회
		$(".btn_eHojoSearch_swgeCntrwkRegstrAdd", that.selector).on("click", function() {
			that.ehojo();
			return false;
		});
		
		// 저장
		$(".btn_save_swgeCntrwkRegstrAdd", that.selector).on("click", function() {
			that.save();
	...
	open : function() {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/addSwgeCntrwkRegstrPage.do";
		var windowOptions = {
			width : 680,
			height : 579,
			top : 120,
	...
	ehojo : function() {
		...
		if(cttNum != ""){
			var url = CONTEXT_PATH + "/cntrwkRegstr/swge/" + cttNum + "/ehojoInfo.do";
	...
	save : function() {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/addSwgeCntrwkRegstr.do";
		$("form", that.selector).attr("action", url);
		$("form", that.selector).submit();
	},
...
swgeCntrwkRegstrObj.addPhoto = {
		
	TITLE : "하수 공사대장 사진 등록",
	
	CLASS_NAME : "SwgeCntrwkRegstrPhotoAdd",
		...
		$("form", that.selector).ajaxForm({
			...
			success : function(result) {
				if(result) {
					...
					that.close();
					alert("하수공사대장 공사사진이 등록되었습니다.");
				}
				else {
					alert("하수공사대장 공사사진 등록이 실패하였습니다.");
				}
			}
		});
	},
	...
	bindEvents : function() {
		...
		// 등록
		$("#a_save_swgeCntrwkRegstrPhotoAdd", that.selector).on("click", function() {
			that.save();
	...
	open : function(ftrIdn) {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/addSwgeCntrwkRegstrPhotoPage.do";
		var windowOptions = {
			width : 430,
			height : 279,
			top : 120,
	...
	save : function() {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/" + that.ftrIdn + "/addSwgeCntrwkRegstrPhoto.do";
		$("form", that.selector).attr("action", url);
		$("form", that.selector).submit();
	},
...
//하수 공사대장 사진 상세조회
swgeCntrwkRegstrObj.viewPhoto = {
		
	TITLE : "하수 공사대장 사진정보",
	
	CLASS_NAME : "SwgeCntrwkRegstrPhotoView",
	...
	open : function(imageNo) {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/" + that.imageNo + "/selectOneSwgeCntrwkRegstrPhotoPage.do";
		var windowOptions = {
			width : 445,
			height : 529,
			top : 120,
...
//하수 공사대장 사진 수정
swgeCntrwkRegstrObj.modifyPhoto = {
		
	TITLE : "하수 공사대장 사진 수정",
	
	CLASS_NAME : "SwgeCntrwkRegstrPhotoModify",
		...
		$("form", that.selector).ajaxForm({
			...
			success : function(result) {
				if(result) {
					swgeCntrwkRegstrObj.modify.open(that.ftrIdn);
					that.close();
					alert("하수공사대장 공사사진이 수정되었습니다.");
				}
				else {
					alert("하수공사대장 공사사진 수정이 실패하였습니다.");
				}
			}
		});
	},
	...
	bindEvents : function() {
		...
		// 수정
		$("#a_save_swgeCntrwkRegstrPhotoModify", that.selector).on("click", function() {
			that.save();
	...
	open : function(imageNo, ftrIdn) {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/" + that.imageNo + "/modifySwgeCntrwkRegstrPhotoPage.do";
		var windowOptions = {
			width : 430,
			height : 279,
			top : 120,
	...
	save : function() {
		...
		var url = CONTEXT_PATH + "/cntrwkRegstr/swge/" + that.ftrIdn + "/" + that.imageNo + "/modifySwgeCntrwkRegstrPhoto.do";
		$("form", that.selector).attr("action", url);
		$("form", that.selector).submit();
	},
