windowObj.registerObj = {
	...
	open : function(dataId, fids, feature, onSave, onLoad) {
		...
		that.loadDataFieldCategory().done( 
				function(response) {
					...
					var url = CONTEXT_PATH + "/window/register/page.do";
					var windowOptions = {
						width : 380,
						height : 600,
						tools : "#div_window_register_tools",
						onClose : function() {
							that.close();
						},
						resizable : true
	...
	initUi : function() {
		$("#div_window_register_tools_prev").hide();
	},
	...
	checkAuthor : function() {
		var that = this;
		var dataAuthor = dataAuthorObj.getDataAuthor(that.dataId);
		var params = { sysId : SYS_ID };

		if (that.data.prntngAt == 'Y' && dataAuthor && dataAuthor.prntngAt == 'Y') {
		...
		if (dataAuthor.dataId == 'RDL_SECT_LS') {
			...
			// 영상관리
			$(".a_video_manage", that.selector).show();
			// 구간도면 관리
	...
	setMode : function(mode) {
		...
		if (mode == "view") {
			if (that.index == 0) {
				...
				$("#div_window_register_tools_prev").show();
			}

			if (that.index == that.fids.length - 1) {
				$("#div_window_register_tools_next").hide();
			} else {
				$("#div_window_register_tools_next").show();
			}
	...
	bindEvents : function() {
		...
		// 이전
		$("#div_window_register_tools_prev").click(function() {
			that.index--;
			$("#div_window_register_tools_next").show();
			if (that.index == 0) {
				$("#div_window_register_tools_prev").hide();
		...
		// 다음
		$("#div_window_register_tools_next").click(function() {
			that.index++;
			$("#div_window_register_tools_prev").show();
			if (that.index == that.fids.length - 1) {
				$("#div_window_register_tools_next").hide();
		...
		// 영상관리
		$(".a_video_manage", that.selector).click(function() {
			that.videoManage();
			return false;
		});
		...
		$(".a_edit_update", that.selector).click(function() {
			if(that.feature) {
				...
				//보호구역
				else if(that.dataId == "RDL_PRTS_AS"){
					that.update();
					that.onSave();
				}
				else {
		...
		// 측량표지현황조사이력
		$(".a_mesrSgnalSttusExaminHist", that.selector).click(function() {
			var ftfCde = that.getFtrCde();
			var ftfIdn = that.fids;
						...
						if (category.fieldCtgryId == dataField.fieldCtgryId) {
							...
							if (that.feature && that.feature.getProperties() && that.feature.getProperties()[dataField.fieldId] != null) {
								...
								if (dataField.fieldId == "FTR_IDN") {
									...
									if (dongObj.getDataId() != '') {
										value = that.getDefaultSpatialValue(dongObj);
									}
								} else if (dataField.fieldId == "HJD_CDE") {
									if (hDongObj.getDataId() != '') {
										value = that.getDefaultSpatialValue(hDongObj);
									}
								} else if (dataField.fieldId == "SHT_NUM") {
									if (index1000Obj.getDataId() != '') {
										value = that.getDefaultSpatialValue(index1000Obj);
									}
								} else if (dataField.fieldId == 'FTF_IDN'){
									...
									var wkt = format.writeFeature(new ol.Feature(gisObj.totransformCooridateSystem(bufferGeometry)));
									var filter = {
											filterType : 'SPATIAL',
									...
									if(dataId == 'WTL_META_PS'){
										layerName = 'WTL_SPLY_LS';
									}else if(dataId == 'WTL_VALV_PS' || dataId == 'WTL_FLOW_PS' || dataId == 'WTL_PRGA_PS' || dataId == 'WTL_LEAK_PS'){
										layerName = 'WTL_PIPE_LM';
									}
									
									$.ajax({
										url : CONTEXT_PATH + '/rest/spatial/' + layerName + '/listAll.do',
										dataType : 'json',
										data : filter,
										async : false,
										type : 'POST',
							...
							if (indictTy != "HIDE" && indictTy != "WKT") {
								if (category.fieldCtgryId == dataField.fieldCtgryId) {
									...
									if (that.feature && that.feature.getProperties() && that.feature.getProperties()[dataField.fieldId] != null) {
										...
										/*//보호구역
										if(dataField.dataId.indexOf("RDL_PRTS_AS") > -1){
											if(dataField.fieldId.indexOf("FAC_NUM") > -1 || dataField.fieldId.indexOf("FAD_NUM") > -1){
												value = stringUtils.lpad(value, 4);
											}
										}*/
									}
			...
			isOriginColumnValue : true
				...
				function(data) {
					if (data) {
						...
						url += that.data.roadRegAt + "/" + that.dataId
									+ "/roadRegstrPrint.do?" + "ftrCde=" + data.ftrCde
									+ "&" + "ftrIdn=" + data.ftrIdn;
						popupUtils.open(url, that.data.roadRegAt, {
							width : 900,
							height : 800,
							left : 300,
							top : 150
	...
	unitedRoadRegstrPrint : function() {
		...
		function(data) {
			if (data) {
				url += that.data.roadRegAt + "/unitedRoadRegstrPrint.do?rotNam=" + data.rotNam + "&rotIdn=" + data.rotIdn + "&secIdn=" + data.secIdn;
	...
	manageHistory : function() {
		...
		spatialSearchUtils.selectOne(title, dataId, fid, {
			isOriginColumnValue : true
		}, function(data) {
			if (data) {
	...
	videoManage : function() {
		...
		spatialSearchUtils.selectOne(that.title, that.dataId, that.getFid(), {
			...
			if (data) {
				...
				windowObj.videoManageObj.open(ftrIdn);
			}
		});
	},
	...
	sectionPlan : function() {
		...
		spatialSearchUtils.selectOne(that.title, that.dataId, that.getFid(), {
			...
			if (data) {
				...
				windowObj.sectionPlanObj.open(ftrIdn);
			}
		});
	},
	...
	localPlan : function() {
		...
		spatialSearchUtils.selectOne(that.title, that.dataId, that.getFid(), {
			...
			if (data) {
				...
				windowObj.localPlanObj.open(ftrIdn);
			}
		});
	},
								...
								if(sysTy == "COMMON" || sysTy == "") {
									if(indictTy != "HIDE" && indictTy != "WKT") {
										if(category.fieldCtgryId == dataField.fieldCtgryId) {
											...
											/*//보호구역
											if(dataField.dataId.indexOf("RDL_PRTS_AS") > -1){
												if(field.indexOf("facNum") > -1 || field.indexOf("fadNum") > -1){
													value = stringUtils.lpad(value, 4);
												}
											}*/
	...
	getEditor : function(dataField) {
		...
		var editable = dataField.editAt == "Y" ? true : false;
		...
		} else if (indictType == "DATE_FROM_STRING") {
			editor = {
					type : "datebox",
					options : {
						required : required,
...
//				break;
	...
	createCategoryDatagrid : function() {
		...
		for ( var id in that.categories) {
			...
			str += "<table class='tbl_register_detail " + id + "' style='width:100%'></table>";
			str += "</div>";
		}
		// 태그 삽입
	...
	update : function() {
		...
		if (!props) {
			...
			if(PROJECT_CODE == "gn") {
				var oKeys = Object.keys(props);
				
				if(oKeys.indexOf("FAC_NUM") > -1){
					props["FAC_NUM"] = stringUtils.lpad(props["FAC_NUM"], 4);
				}
				
				if(oKeys.indexOf("FAD_NUM") > -1){
					props["FAD_NUM"] = stringUtils.lpad(props["FAD_NUM"], 4);
				}
			}
		}
