...
landUseInfoRegstrObj = {
	...
	bindEvents : function(){
		...
		$(".a_reset_landUseInfoRegstr", that.selector).on("click", function(){
			...
			///초기화 
			$("#selDong", that.selector).combobox('select',"4215000000");
			$("#listLandUseInfoRegstr .easyui-numberbox", that.selector).numberbox('clear');
			$(".chk_mntn").attr('checked', false);
...
landUseInfoRegstrObj.view = {
	...
	init : function(){
		...
		$("#a_sealImpression").hide();
		$("#a_siteMap").hide();
		$("#a_etc").hide();
		
		var cntIdn = $("#cntIdn", that.selector).val();
		var luiIdn = $("#luiIdn", that.selector).val();
		var pnu = $("#pnu", that.selector).val();
		var url = CONTEXT_PATH + "/landUseInfo/" + cntIdn + "/" + luiIdn + "/findLandFileNm.do";
		
		$.get(url).done(function(result){
			if(result && result.result.length > 0){
				for(var i=0; i < result.result.length; i++){

					if(result.result[i].docCde == "AGR"){
						$("#a_writtenConsentOfUse").show();
						$("#a_writtenConsentOfUse").attr("href", CONTEXT_PATH + "/landUseInfo/" + pnu + "/AGR/" + result.result[i].docFile + "/landDownload.do");
						$(".btn_landUseInfo_writtenConsentOfUse").val(result.result[i].docFile);
					}
					else if(result.result[i].docCde == "REG"){
						$("#a_sealImpression").show();
						$("#a_sealImpression").attr("href", CONTEXT_PATH + "/landUseInfo/" + pnu + "/REG/" + result.result[i].docFile + "/landDownload.do");
						$(".btn_landUseInfo_sealImpression").val(result.result[i].docFile);
					}
					else if(result.result[i].docCde == "SIT"){
						$("#a_siteMap").show();
						$("#a_siteMap").attr("href", CONTEXT_PATH + "/landUseInfo/" + pnu + "/SIT/" + result.result[i].docFile + "/landDownload.do");
						$(".btn_landUseInfo_siteMap").val(result.result[i].docFile);
					}
					else if(result.result[i].docCde == "ETC"){
						$("#a_etc").show();
						$("#a_etc").attr("href", CONTEXT_PATH + "/landUseInfo/" + pnu + "/ETC/" + result.result[i].docFile + "/landDownload.do");
						$(".btn_landUseInfo_etc").val(result.result[i].docFile);
					}
				}
			}
		});
...
landUseInfoRegstrObj.modify = {
	...
	bindEvents : function(){
		...
		// pnu 매칭
		var pnuResult = $("#landUseInfoRegstrModifyformPnu", that.selector).val();
	...
	modify : function(){
		...
		$("#landUseInfoRegstrModifyformPnu").val(pnuCode);
...
landUseInfoRegstrObj.quick = {
	...
	bindEvents : function(){
		...
		// 닫기 버튼
		$(".btn_close_landUseInfoList", that.selector).on("click", function(){
			that.close();
	...
	open : function(pnu){
		...
		var url = CONTEXT_PATH + "/landUseInfo/" + pnu + "/landUseInfoQuick.do";
		...
		$.get(url).done(function(response) {
			...
			$.messager.alert(that.TITLE, "토지사용정보 리스트를 가져오는데 실패하였습니다.");
		});
	},
