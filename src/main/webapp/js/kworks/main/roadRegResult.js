...
var roadRegResultObj = {
	...
	getDataPkColumn : function() {
		var that = this;
		return that.gridOptions[that.data.regIdn].pk;
	},
	...
	addRows : function(data) {
		...
		// 도로 대장 검색 에서 기존목록 있을시 닫고 실행
		that.close();
		roadVideoResultObj.close();
		...
		// 중복된 데이터(KWS_ROAD_REG)가 있는 경우 선택창 표시
		if (isExist) {
	...
	/**
	 * 목록 추가 - 중복된 데이터 아이디(KWS_ROAD_REG)가 있는 경우
	 * 
	 * @param data
	 */
...
//		var dataAuthor = dataAuthorObj.getDataAuthor(dataId);
	 ...
	 *            regIdn 도로대장 아이디
	 */
	createGridOptions : function(regIdn) {
		var that = this;
...
//			}
			...
			for (var i = 0; i < data.featureIds.length; i++) {
				...
				if (data.featureIds[i].ftrIdn) {
					ftrIdn.push(data.featureIds[i].ftrIdn);
				}
			}
	...
	searchRegister : function() {
		...
		if(selects.length > 0) {
			...
			windowObj.roadRegisterObj.open(regIdn, featureInfos, dataFields);
		} else {
