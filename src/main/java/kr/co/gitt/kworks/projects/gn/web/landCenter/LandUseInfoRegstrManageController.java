...
import kr.co.gitt.kworks.projects.gn.service.ladCntr.LandCenterUseDownLogService;
import kr.co.gitt.kworks.projects.gn.service.ladCntr.LandCntrwkUseFileService;
...
public class LandUseInfoRegstrManageController {
	...
	public String searchLandUseInfoRestrPage(LdUseMa ldUseMa, Model model){
		...
		return "/projects/gn/job/landCenter/listLandUseInfoRegstr";
	}
	...
	public String selectOneLandUseInfoRegstr(@PathVariable("luiIdn") Long luiIdn, Model model) throws Exception{
		...
		return "/projects/gn/job/landCenter/viewLandUseInfoRegstr";
	}
	...
	public String modifyLandUseInfoRegstrPage(@PathVariable("luiIdn") Long luiIdn,@PathVariable("cntIdn") Long cntIdn, LdConsMa ldConsMa, Model model){
		KwsDomnCode kwsDomnCode = new KwsDomnCode();
		kwsDomnCode.setSearchCondition("2");
		kwsDomnCode.setDomnId("KWS-0199");
		...
		model.addAttribute("selDong", domnCodeService.listDomnCode(kwsDomnCode));
		model.addAttribute("cntNamList", ladCntrwkRegstrService.cntSelectOneLandCenterCetrwkRegstr(cntIdn));
		...
		return "/projects/gn/job/landCenter/modifyLandUseInfoRegstr";
	}
	...
	public String modifyLandUseInfoRegstr(@PathVariable("luiIdn") Long luiIdn, @PathVariable("cntIdn") Long cntIdn, LdUseMa ldUseMa, MultipartRequest multipartRequest, Model model) throws FdlException, IOException {
	...
	public String addLandUseInfoRegstrPage(LdConsMa ldConsMa, LdUseMa ldUseMa, Model model){
		KwsDomnCode kwsDomnCode = new KwsDomnCode();
		kwsDomnCode.setSearchCondition("2");
		
		kwsDomnCode.setDomnId("KWS-0199");
		model.addAttribute("selDong", domnCodeService.listDomnCode(kwsDomnCode));
		model.addAttribute("cntNam", ladCntrwkRegstrService.listAllLandCenterCetrwkRegstr(ldConsMa));
		
		return "/projects/gn/job/landCenter/addLandUseInfoRegstr";
	}
	...
	///~~~~~~~~~~~~~
	/////////////////////////////////////////////
	@RequestMapping(value="{pnu}/landUseInfoQuick.do", method=RequestMethod.GET)
	public String landUseInfoQuick(@PathVariable("pnu") String pnu, Model model){
		model.addAttribute("result", ladCntrwkRegstrService.landUseInfoQuickSearch(pnu));
	...
	///~~~~~~~~~~~~~
	/////////////////////////////////////////////
	@RequestMapping(value="{cntIdn}/{luiIdn}/findLandFileNm.do", method=RequestMethod.GET)
	public String landDocMaFile(@PathVariable("cntIdn")Long cntIdn, @PathVariable("luiIdn") Long luiIdn, Model model){
		model.addAttribute("result", ladCntrwkRegstrService.landDocFileList(cntIdn, luiIdn));
	...
	/////////////////////////////////////////////
	/// @fn download
	/// @brief 함수 간략한 설명 : 토지사용증명서 파일 다운로드
	/// @remark
	/// - 함수의 상세 설명 : 
	/// @param streFileNm
	/// @param response
	...
	///~~~~~~~~~~~~~
	/////////////////////////////////////////////
	@RequestMapping(value="{pnu}/{docCde}/{streFileNm}/landDownload.do")
	public void landDownload(@PathVariable("pnu")String pnu, @PathVariable("docCde")String docCde, @PathVariable("streFileNm")String streFileNm, HttpServletResponse response, @RequestHeader(value="User-Agent") String userAgent) throws Exception {
		KwsFile kwsFile = fileService.selectOneLandFile(streFileNm);
		InputStream is = new FileInputStream(kwsFile.getFileStreCours() + kwsFile.getStreFileNm());
		
		String fileName = kwsFile.getStreFileNm();
		String docName = null;
		...
		LdDocMa ldDocMa = new LdDocMa();
		ldDocMa.setDocCde(docCde);
		ldDocMa.setDocFile(streFileNm);
		ldDocMa.setPnu(pnu);
