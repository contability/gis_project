 ...
 * 공간편집
 */
windowObj.reantEditObj = {
	
	selector : null,
	
	TITLE : "공간편집",
	
	CLASS_NAME : "ReplantEdit",
	...
	initGis : function() {
		...
		interaction.set("id", "drawCircle2");
		interaction.set("name", "drawCircle2");
		...
		translate.set("id", "circleTranslate2");
		translate.set("name", "circleTranslate2");
	...
	bindEvents : function() {
		...
		// 신규등록
		$("#a_replant_regist").click(function() {
			windowObj.reantPropertyObj.close();		
			that.regist();
		});
		
		// 선택
		$("#a_replant_select").click(function() {
			windowObj.reantPropertyObj.close();		
			that.select();
		});
		
		// 신규등록 - 그리기
		$("#a_replant_draw").click(function() {
			that.draw();
			$("#a_property_save").show();
			$("#a_replant_save").show();
		});
		
		// 선택 - 이동
		$("#a_replant_move").click(function(evt) {
			that.move(evt);
			$("#a_replant_save").show();
		});
		
		//속성편집
		$("#a_property_save").click(function(evt) {
			if(that.feature){
				var layerId = "REANT_AS";
				...
				if(that.srcProperties){
					that.properties["areaGs"] = that.srcProperties["areaGs"];
					that.properties["areaGj"] = that.srcProperties["areaGj"];
					that.properties["areaHn"] = that.srcProperties["areaHn"];
					that.properties["areaJw"] = that.srcProperties["areaJw"];
					that.properties["areaTs"] = that.srcProperties["areaTs"];
				}else{
					that.properties["areaGs"] = 0.0;
					that.properties["areaGj"] = 0.0;
					that.properties["areaHn"] = 0.0;
					that.properties["areaJw"] = 0.0;
					that.properties["areaTs"] = 0.0;
				}
				var selDongSearch = that.properties.pnu.substring(0, 8);
				if(selDongSearch=="42820250"){
					that.properties["areaGs"] = that.featureArea;
				}else if(selDongSearch=="42820320"){
					that.properties["areaGj"] = that.featureArea;
				}else if(selDongSearch=="42820253"){
					that.properties["areaHn"] = that.featureArea;
				}else if(selDongSearch=="42820310"){
					that.properties["areaJw"] = that.featureArea;
				}else{
					that.properties["areaGs"] = that.featureArea;
				}
				var isNew = false;
				...
				windowObj.reantPropertyObj.open(layerId, that.properties, isNew); // 상세 조회창
			} else {
		...
		// 저장
		$("#a_replant_save").click(function() {
			that.save();
			return false;
		});
		
		// 닫기
		$("#a_replant_close").click(function() {
			that.close();
		});
	},
	...
	open : function() {
		...
		that.layerName = "제거작업일지";
		that.drawType = "Circle";
		
		mapObj.reloadWMS();
		
		var url = "/enviro/window/reantEditPage.do";
		var windowOptions = {
			title : that.TITLE,
			width : 450,
			height : 180,
		...
		var id = windowObj.createWindow(that.CLASS_NAME, that.TITLE, url, null, windowOptions, function() {
			...
			$("#td_edit_replant_title").text("생태교란 외래식물 제거작업일지");
		});
		that.selector = "#" + id;		
	...
	regist : function() {
		...
		$("#a_replant_draw").show();
		$("#a_replant_draw img").trigger("click");
	...
	addFeature : function(evt) {
		...
		if (geometry) {
			...
			if (that.isDraw) {
				...
				var selDongSearch = that.properties.pnu.substring(0, 8);
				switch(selDongSearch)
				{
				case "42820250":
					that.featureArea = that.properties["areaGs"];
					break;
					
				case "42820320":
					that.featureArea = that.properties["areaGj"];
					break;
					
				case "42820253":
					that.featureArea = that.properties["areaHn"];
					break;
					
				case "42820310":
					that.featureArea = that.properties["areaJw"];
					break;
	...
	draw : function() {
		...
		mapObj.getMap().activeInteractions("drag", "drawCircle2");
		var geometry = that.feature?that.feature.getGeometry():null;
		that.coords.viewCoords(geometry, that.address, that.featureArea);
	...
	drawstart : function(evt) {
		var editLayer = mapObj.getMap().getLayer("editLayer2");
		...
		this.overlay.set("name", "editOverlay2");
	...
	search : function(geom) {
		...
		spatialSearchUtils.listAll(that.TITLE, "REANT_AS", filter, function(rows) {
			if (rows && rows.length > 0) {
				...
				if (rows.length > 0) {
					...
					var pkColumn = dataObj.getPkColumn("REANT_AS");
				...
				$("#a_edit_remove").show();
				$(".div_edit_coords", that.selector).show();
				$("#a_replant_move img").trigger("click");
...
//			$(".div_edit_draw", that.selector).show();
			...
			var translate = mapObj.getMap().getInteraction("circleTranslate2");
			translate.setFeatures(features);
			mapObj.getMap().activeInteractions("drag", "circleTranslate2");
			...
			$.messager.alert(that.TITLE, "선택된 작업지역이 없습니다.");
			evt.stopPropagation();
		}
	},
	...
	translateend : function(evt) {
		...
		if (geometry) {
			var center = spatialUtils.calculateCenter(geometry);
			var address = that.getAddress(center);
	...
	getAddress : function(geometry) {
		var that = this;
		var format = new ol.format.WKT();
		var wkt = format.writeFeature(new ol.Feature(geometry));
		...
		if(that.feature) {
			...
			if (!that.isSelected) {	// 신규
				if (that.properties == null) {
					...
					var dtData = windowObj.reantPropertyObj.getProperties();

					that.feature.values_.FTR_CDE = "RE001";
					that.feature.values_.EXP_IDN = dtData["expIdn"];
					that.feature.values_.FTR_IDN = dtData["ftrIdn"];
					that.feature.values_.JOB_TYPE = dtData["jobType"];
					that.feature.values_.JOB_PERS = dtData["jobPers"];
					that.feature.values_.JOB_NOTE = dtData["jobNote"];
					that.feature.values_.AREA_GS = dtData["areaGs"];
					that.feature.values_.AREA_HN = dtData["areaHn"];
					that.feature.values_.AREA_JW = dtData["areaJw"];
					that.feature.values_.AREA_GJ = dtData["areaGj"];
					that.feature.values_.AREA_TS = dtData["areaTs"];
					that.feature.values_.ETC_NOTE = dtData["etcNote"];
					that.feature.values_.JOB_DATE = that.properties["jobDate"];
					that.feature.values_.PNU = that.properties["pnu"];
					
					var expIdn = dtData["expIdn"];
					var jobType = dtData["jobType"];
					
					$.ajax({
						url : CONTEXT_PATH + "/enviro/"+ expIdn + "/" + jobType + "/reantExpIdnSearch.do",
						async : false,
						type : "GET",
						success : function(data) {
							if(data.result.length>0){
								$.messager.alert(that.TITLE, "같은작업 내용을 등록할수 없습니다.");
								that.close();
								that.clear();
								mapObj.reloadWMS();
							} else{
			...
			//편집
				...
				if (that.properties == null) {
					...
					that.feature.values_.EXP_IDN = dtData["expIdn"];
					that.feature.values_.FTR_IDN = that.properties["ftrIdn"];
					that.feature.values_.JOB_TYPE = dtData["jobType"];
					that.feature.values_.JOB_PERS = dtData["jobPers"];
					that.feature.values_.JOB_NOTE = dtData["jobNote"];
					that.feature.values_.AREA_GS = dtData["areaGs"];
