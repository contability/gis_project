...
var ugrwtrObj = {
	...
	bindEvents : function() {
		...
		$("div", that.selector).on("click", ".btn_init4", function() {
			...
			$("select[name=dvopLocN]", that.selector).val("");
	...
	search : function() {
		...
		// 법정동
		params.dvopLocN = $("select[name=dvopLocN]", that.selector).val();
		// 용도구분
...
ugrwtrObj.view = {
	...
	bindEvents : function() {
		...
		$(".a_open_chckImprmnHist_viewUgrwtr", that.selector).on("click", function() {
			...
			ugrwtrObj.chckImprmnHist.open(ftrIdn);
		});
...
ugrwtrObj.chckImprmnHist = {
	...
	bindEvents : function() {
		...
		$(".a_add_listUgrwtrChckHist", that.selector).on("click", function() {
			...
			ugrwtrObj.chckImprmnHist.add.open(ftrIdn);
		});
	...
	open : function(ftrIdn) {
		...
		var url = CONTEXT_PATH + "/ugrwtr/chckImprmnHist/" + ftrIdn + "/page.do";
		var windowOptions = {
			width : 600,
			height : 358,
			top : 120,
...
ugrwtrObj.chckImprmnHist.add = {
	...
	init : function() {
		...
		$("form", that.selector).ajaxForm({
			...
			success : function(result) {
				if(result) {
					that.close();
					ugrwtrObj.chckImprmnHist.open(that.ftrIdn);
				}
				else {
	...
	save : function() {
		...
		var url = CONTEXT_PATH + "/ugrwtr/chckImprmnHist/" + that.ftrIdn + "/add.do";
