...
<head>
	...
	<c:set var="sysCount">${fn:length(sysAuthorList) + 1}</c:set>	<!-- 동해시 타 시스템 링크 추가를 위해 -->
	...
	<div id="wrap">
		...
		<div id="container">
			...
			<div class="slider bd_t_blue4 bd_b_gray3">
				<div class="slideMap m_auto">
					<ul class="kworksMainImg">
						<li>
							<img src="<c:url value='/images/${prjCode}/portal/main_1.png' />" alt="동해 이미지 1" />
						</li>
						<li>
							<img src="<c:url value='/images/${prjCode}/portal/main_2.png' />" alt="동해 이미지 2" />
						</li>
						<li>
							<img src="<c:url value='/images/${prjCode}/portal/main_3.png' />" alt="동해 이미지 3" />
						</li>
					</ul>
				</div>
			</div>
			...
			<div class="w100 bg_f2">
				...
				<c:if test="${isMySystem ne true}" >
					<div style="height : 20px;"></div>
				</c:if>
				<div class="survice2 t_a_c">
					<c:if test="${sysCount > 5}">
					...
					<div class="f_l w_1000">
						...
						<ul id="portal_sys_slider">
								...
								<c:forEach items="${sysAuthorList}" var="sysAuthor">
									<c:if test="${sysAuthor.sysId eq sys.sysId}">
										<li>
											...
											<c:if test="${sys.sysId eq '7' }">
												<a href="#" onclick="alert('서비스 준비중입니다.');return false;">
													<img class="mainSys cursor_point" src="<c:url value='/images/${prjCode}/portal/sys_${sys.sysNm}.png' />" />
												</a>
											</c:if>
							...
							<!-- 동해시 인허가 의사결정지원시스템 링크 -->
							<li><a href="http://106.4.1.42:9904/LDSS/home/link-to-login.ldss?userId=<c:out value='${userId}' />"><img class="cursor_point" src="<c:url value='/images/${prjCode}/portal/sys_LINK_LDSS.png' />" /></a></li>
						</ul>
					</div>
					<c:if test="${sysCount > 5}">
