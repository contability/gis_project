...
<form id="localPlanModifyform" name="localPlanModifyform" method="post" enctype="multipart/form-data" >
	...
	<div id="div_window_section_plan" class="layerCont">
		<table class="cmmTable v2 ma_b_30" summary="단위도면  수정 테이블">
			<caption>단위도면 수정 테이블</caption>
			<colgroup>
				<col width="22%" />
				<col width="28%" />
				<col width="25%" />
			...
			<tbody>
				...
				<tr>
					<th>구간이정</th>
					<td>
						<input type="text" id="secDis" name="secDis" class="w100" value="${result.secDis}" />
					</td>
					<th>도면</th>
					<td>
						<select name="lclCde" class="selectBox w100">
							<c:forEach items="${lclCde}" var="lclCde" varStatus="status">
								<option value="${lclCde.codeId}" ${lclCde.codeId==result.lclCde ? 'selected' : ''}>${lclCde.codeNm}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row">단위도면 파일</th>
					<td colspan="3">
						<p class="ma_b_10">
							<input type="file" name="localPlanFile" id="localPlanFile" value="" accept="video/mp4|image/*">
						</p>
					</td>
				</tr>
				</tbody>
			...
			<a class="a_save_modify_local_plan btn_register" href="#" ></a>
			<a class="a_close_modify_local_plan btn_close" href="#" ></a>
		</div>
	</form>
</div>
