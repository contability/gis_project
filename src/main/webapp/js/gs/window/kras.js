...
var krasObj = {
	...
	//세움터 건물 정보 대장 검색
	DjrBldRgstList : function(){
		var that = this;
		
		var url = CONTEXT_PATH + "/contact/eais/bildngPrmisnRegstr/DjrBldRgstList.do";
		...
		$.get(url, params).done(function(response) {
			if(response){
				if( response.data.length > 0 ){
					that.data.bldgInfo = response;
					that.loadBldg();
				}else{
					$(".bldg_info", that.selector).hide();
				}
			}else{
				$.messager.alert(that.TITLE, "건물 정보를 조회 하는데 실패했습니다.");
				that.hideLoadingBar();
			}
		}).fail(function() {
			$.messager.alert(that.TITLE, "건물 정보를 조회 하는데 실패했습니다.");
	...
	searchBldg : function() {
		...
		// KRAS 서비스 호출
		var bldrgstPk = $(".tby_kras_dong_info tr.on").attr("data-pk-no");
		var regstrKindCd = $(".tby_kras_dong_info tr.on").attr("data-kind-cd");
		var violBldYn = $(".tby_kras_dong_info tr.on").attr("viol-bld-yn");
		...
		if(bldrgstPk && regstrKindCd) {
			var url = CONTEXT_PATH + "/contact/eais/bildngPrmisnRegstr/"+bldrgstPk+"/"+regstrKindCd+"/selectDjrTitle.do";
			var params = {
				pnu : that.pnu
			};
			...
			$.get(url, params).done(function(result) {
				if(result){
							...
							var djrTitle = result.data;
							
							//위반 건축 여부
							$(".tbl_kras_bldg_info .violBldYn", that.selector).text(violBldYn);
							
							//건축물 정보
							for(var i = 0; i < Object.keys(djrTitle).length; i++){
								var key = Object.keys(djrTitle)[i];
								
								$(".tbl_kras_bldg_info ."+key, that.selector).text(djrTitle[key]);
							}
								
							
							// 변동 현황
							if(djrTitle.djrChangItemList) {
								var djrChangItemList = djrTitle.djrChangItemList;
								var tagStr = "";

								for(var i in djrChangItemList) {
									tagStr += "<tr>";
									tagStr += "<td>" + djrChangItemList[i].changCausCdNm + "</td>";
									tagStr += "<td>" + djrChangItemList[i].changDetl + "</td>";
									tagStr += "<td>" + djrChangItemList[i].changDay + "</td>";
									tagStr += "<td>" + djrChangItemList[i].adjstDay + "</td>";
									tagStr += "</tr>";
								}
							...
							// 층별 현황
							if(djrTitle.djFlroulnList) {
								var djFlroulnList = djrTitle.djFlroulnList;
								var tagStr = "";

								for(var i in djFlroulnList) {
									tagStr += "<tr>";
									tagStr += "<td>" + djFlroulnList[i].flrNoNm + "</td>";
									tagStr += "<td>" + djFlroulnList[i].strctCdNm + "</td>";
									tagStr += "<td>" + djFlroulnList[i].mainPurpsCdNm + "</td>";
									tagStr += "<td>" + djFlroulnList[i].area + "</td>";
									tagStr += "</tr>";
								}
	...
	loadBldg : function() {
		...
		var bldgDongInfos = that.data.bldgInfo.data;
		for(var i=0, len=bldgDongInfos.length; i < len; i++) {
			var bldgDongInfo = bldgDongInfos[i];
			tagStr += '<tr data-pk-no="'+bldgDongInfo.bldrgstPk+'" data-kind-cd="'+bldgDongInfo.regstrKindCd+'" viol-bld-yn="'+bldgDongInfo.violBldYn+'" >';
			tagStr += '<td>' + bldgDongInfo.regstrKindCdNm + '</td>';
			tagStr += '<td>' + bldgDongInfo.bldrgstPk + '</td>';
			tagStr += '<td>' + bldgDongInfo.bldNm + '</td>';
