...
import com.clipsoft.org.apache.batik.css.engine.value.StringValue;

import kr.co.gitt.kworks.cmmn.view.AbstractExcelView;
import kr.co.gitt.kworks.model.KwsSys;

@Component("weekIndvdlzSysConectStatsExcelDownload")
public class WeekIndvdlzSysConectStatsExcelDownload extends AbstractExcelView {
...
//		setText(getCell(sheet, 0, sysList.size() + 1), "합계");
		...
		for(int i=0; i < categories.size(); i++ ) {
			...
			cell.setCellValue(String.valueOf(categories.get(i).get("totalCount").toString()));
			
			for(int j = 0; j < sysList.size(); j++){
				cell = getCell(sheet, (i  + 1), (j + 2));
				cell.setCellValue(String.valueOf(categories.get(i).get("sys" + sysList.get(j).getSysId() + "Count")));
			}
