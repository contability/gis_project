cmmnPropObj.drawToolObj = {
		...
		initGis : function() {
			...
			cmmnLoanObj.view.getMap().addLayer(layer);
			...
			for(var i in types) {
				...
				cmmnLoanObj.view.getMap().addInteraction(interaction);
			...
			cmmnLoanObj.view.getMap().addInteraction(modify);
...
//			cmmnLoanObj.view.getMap().addInteraction(translate);
...
//			$(".div_edit_coords", that.selector).show();
			cmmnLoanObj.view.getMap().activeInteractions("drag", "editLoan"+that.drawType);
...
//			that.clear();
			...
			if(geometry) {
				...
				var mapProj = cmmnLoanObj.view.getMap().getView().getProjection();
				if(proj != mapProj) {
...
//				that.showSeq();
		...
		modify : function(evt) {
			...
			if(that.feature) {
				...
				var modify = cmmnLoanObj.view.getMap().getInteraction("editLoanModify");
				modify.setFeatures(features);
				cmmnLoanObj.view.getMap().activeInteractions("drag", "editLoanModify");
//				$(".div_edit_markup", that.selector).hide();
//				$(".div_edit_coords", that.selector).show();
				var geometry = that.feature?that.feature.getGeometry():null;
//				that.coords.show(that.drawType, geometry);
...
//			$("#tby_edit_coords").html("");
		...
		open : function(prtIdn){
			...
			if(that.feature){
				...
				var url = CONTEXT_PATH + '/cmmnprop/insertLousPage.do';
		...
		initUi : function(){
			...
			$('#lous_lonTit', that.selector).textbox({
				width : 370
			});
			
			$('#lous_rmkExp', that.selector).textbox({
				multiline : true,
				labelPosition : 'top',
				width : 370,
				height : 340,
		...
		getPkId : function(){
			var that = this;
			var url = CONTEXT_PATH + '/cmmnprop/searchLousPk.do';
...
//					return result;
		...
		getDataList : function(){
			...
			var lonTit = encodeURI($('#lous_lonTit', that.selector).textbox('getValue'));
			var rmkExp = encodeURI($('#lous_rmkExp', that.selector).textbox('getValue'));
			var prtDat = that.getToday();
			var prtIdn = that.prtIdn;
			
			that.feature.setProperties({PRT_IDN : prtIdn, LON_TIT : lonTit, PRT_DAT : prtDat, RMK_EXP : rmkExp});
		...
		insert : function(){
			...
			if (that.feature) {
				...
				var pkId = that.lonIdn;	
				
				that.feature.setProperties({ LON_IDN : pkId });
//				console.log(pkId);
...
//				debugger;
				...
				cmmnLoanObj.view.getMap().getInteraction("editLoanModify").clearFeatures();
				...
				kworks.service.WFS.transaction(url, params, insertFeatures, null, null, function(response) {
					if(response.inserted == 1) {
						...
						cmmnLoanObj.view.geomDataLode(that.prtIdn);
