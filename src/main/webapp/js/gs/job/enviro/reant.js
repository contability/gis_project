...
reantObj = {
	...
	initUi : function() {
		...
		// easyUi설정
		$("#searchReant .easyui-datebox", that.selector).datebox();
		$("#searchReant .easyui-textbox", that.selector).textbox();
		$("#searchReant .easyui-combobox", that.selector).combobox();
		$("#searchReant .easyui-numberspinner", that.selector).numberspinner();
		...
		$.post(url, params).done(function(response) {
            if(response && response.result){
			    ...
			    if(rows.length > 0){
			    	...
			    	alert("제거작업일지 검색결과가 없습니다.");
					return false;
			    }
            }

		}).fail(function() {
			alert("제거작업일지 검색에 실패하였습니다");
		});
	},
...
reantObj.registerObj={
	...
	open : function(fids){
		...
		that.dataId = "REANT_AS";
		that.ftrIdn= fids;
		
		var url = CONTEXT_PATH + "/enviro/" + fids + "/viewReant.do";
		var windowOptions = {
			width : 650,
			top : 120,
			height : 580,
	...
	loadPhoto : function(){
		...
		var url = CONTEXT_PATH + "/enviro/" + that.ftrIdn + "/searchRePhotoList.do";
		$.get(url).done(function(response){
			...
			$(".photoList tr", that.selector).click(function() {
				...
				if(idn && idn.length > 0) {
					...
					reantObj.photoObj.open(that, idn);
				}
			});
		});
	},
	...
	bindEvents : function() {
		...
		$(".btn_prt", that.selector).on("click", function() {
			that.expIdn = $('#expIdn').val();
			that.print(that.expIdn);
			return false;
		});
	...
	addPhoto : function() {
		...
		reantObj.photoAddObj.open(that, that.dataId, that.ftrIdn);
	},
	
	print : function(expIdn){
		var that = this;
		reantObj.printObj.open(that.ftrIdn,expIdn);
		
	},
	
	modify : function(){
		var that = this;
		reantObj.modifyObj.open(that.dataId,that.ftrIdn);
	},
	
	remove : function(){
		var that = this;
		
		$.messager.confirm(that.TITLE, "제거작업일지 정보를 삭제하시겠습니까?", function(isTrue) {
			if(isTrue) {
				var url = CONTEXT_PATH + "/enviro/" + that.ftrIdn + "/removeReant.do";
				
				$.get(url).done(function(result) {
					if(result.type == 'del' && result.state >= 1) {
						$.messager.alert({
							title : that.TITLE,
							msg : "제거작업일지 정보를 삭제하였습니다.",
							fn : function() {
								that.close();
							}
						});
						...
						$.messager.alert(that.TITLE, "제거작업일지 삭제에 실패했습니다.");
					}
				}).fail(function() {
					$.messager.alert(that.TITLE, "제거작업일지 삭제에 실패했습니다.");
				});
...
reantObj.printObj={
	...
	CLASS_NAME : "ReantDocument",
			
	idn : null,
		
	properties : null,
	
	open : function(fids, expIdn){
		var that = this;
		...
		that.ftrIdn = fids;
		var url = CONTEXT_PATH + "/enviro/" + fids + "/printReant.do";
		var windowOptions = {
			left : 650,
			top : 0,
			width : 500,
	...
	initUi : function() {
		...
		// 이전
		$(".a_prev", that.selector).linkbutton({
			disabled : true,
			onClick : function() {
				$("#replant_doc1").show();
				$("#replant_doc2").hide();
				$(".a_prev", that.selector).linkbutton("disable");
				$(".a_next", that.selector).linkbutton("enable");
			}
		});
		
		// 다음
		$(".a_next", that.selector).linkbutton({
			disabled : false,
			onClick : function() {
				$("#replant_doc1").hide();
				$("#replant_doc2").show();
				$(".a_prev", that.selector).linkbutton("enable");
				$(".a_next", that.selector).linkbutton("disable");
			}
		});
	...
	loadImage : function(){
		...
		var expIdn = that.expIdn;
		
		url = CONTEXT_PATH + "/enviro/" + expIdn + "/ftrIdnList.do";
		$.get(url).done(function(response) {
			var rows = response.result;
			//이미지를 불러오기위한 로직
			for(var i=0; i<rows.length; i++){
				
				if(rows[i].jobType=="REM001"){
					$.ajax({
						url : CONTEXT_PATH + "/photoManage/" + rows[i].ftrCde + "/" + rows[i].ftrIdn + "/list.do",
						async : false,
						type : "GET",
						success : function(data) {
							...
							image1 = data.rows;
//							console.log(rows);
							for(var l=0; l<image1.length; l++){
								if(image1[l].imageSj="RE001"){
									var imgDownloadUrl = CONTEXT_PATH + "/cmmn/file/" + image1[l].imageFileNo + "/download.do";
									$("#td_rem_bef").html("<div class='div_img' ><img class='img_style' width='100%' height='100%' src='" + imgDownloadUrl + "' /></div>");
								}else{
									var imgDownloadUrl = CONTEXT_PATH + "/cmmn/file/" + image1[l].imageFileNo + "/download.do";
									$("#td_rem_aft").html("<div class='div_img' ><img class='img_style' width='100%' height='100%' src='" + imgDownloadUrl + "' /></div>");
								}
							}
						}
					});
					
				}else if(rows[i].jobType=="REM002"){
					$.ajax({
						url : CONTEXT_PATH + "/photoManage/" + rows[i].ftrCde + "/" + rows[i].ftrIdn + "/list.do",
						async : false,
						type : "GET",
						success : function(data) {
							...
							image2 = data.rows;
//							console.log(rows);
							for(var l=0; l<image2.length; l++){
								if(image2[l].imageSj="RE001"){
									var imgDownloadUrl = CONTEXT_PATH + "/cmmn/file/" + image2[l].imageFileNo + "/download.do";
									$("#td_mow_bef").html("<div class='div_img' ><img class='img_style' width='100%' height='100%' src='" + imgDownloadUrl + "' /></div>");
								}else{
									var imgDownloadUrl = CONTEXT_PATH + "/cmmn/file/" + image2[l].imageFileNo + "/download.do";
									$("#td_mow_aft").html("<div class='div_img' ><img class='img_style' width='100%' height='100%' src='" + imgDownloadUrl + "' /></div>");
								}
							}
						}
					});
					
				}else if(rows[i].jobType=="REM003"){
					$.ajax({
						url : CONTEXT_PATH + "/photoManage/" + rows[i].ftrCde + "/" + rows[i].ftrIdn + "/list.do",
						async : false,
						type : "GET",
						success : function(data) {
							...
							image3 = data.rows;
//							console.log(rows);
							for(var l=0; l<image3.length; l++){
								if(image3[l].imageSj="RE001"){
									var imgDownloadUrl = CONTEXT_PATH + "/cmmn/file/" + image3[l].imageFileNo + "/download.do";
									$("#td_pla_bef").html("<div class='div_img' ><img class='img_style' width='100%' height='100%' src='" + imgDownloadUrl + "' /></div>");
								}else{
									var imgDownloadUrl = CONTEXT_PATH + "/cmmn/file/" + image3[l].imageFileNo + "/download.do";
									$("#td_pla_aft").html("<div class='div_img' ><img class='img_style' width='100%' height='100%' src='" + imgDownloadUrl + "' /></div>");
								}
							}
						}
					});
					
				}else if(rows[i].jobType=="REM004"){
					$.ajax({
						url : CONTEXT_PATH + "/photoManage/" + rows[i].ftrCde + "/" + rows[i].ftrIdn + "/list.do",
						async : false,
						type : "GET",
						success : function(data) {
							...
							image4 = data.rows;
//							console.log(rows);
							for(var l=0; l<image4.length; l++){
								if(image4[l].imageSj="RE001"){
									var imgDownloadUrl = CONTEXT_PATH + "/cmmn/file/" + image4[l].imageFileNo + "/download.do";
									$("#td_hab_bef").html("<div class='div_img' ><img class='img_style' width='100%' height='100%' src='" + imgDownloadUrl + "' /></div>");
								}else{
									var imgDownloadUrl = CONTEXT_PATH + "/cmmn/file/" + image4[l].imageFileNo + "/download.do";
									$("#td_hab_aft").html("<div class='div_img' ><img class='img_style' width='100%' height='100%' src='" + imgDownloadUrl + "' /></div>");
								}
							}
						}
					});
...
//	imageloading : function(ftrIdn,ftrCde){
//		var that = this;
//		
...
//		});
	...
	print : function() {
		...
		jQuery.get('../css/gs/common/printcontainer.css', function (printcss) {
			...
			jQuery.get('../css/gs/common/reantprint.css', function (replantcss) {
				var replantPrint = replantcss;
				...
				win.document.write(replantPrint);
				win.document.write('</style></head><body style="width:95%; height:95%;">');
				
				var jobContent1 = document.getElementById("replant_doc1").innerHTML;
				var jobContent2 = document.getElementById("replant_doc2").innerHTML;
				
				win.document.write(jobContent1);
				win.document.write(jobContent2);
...
reantObj.modifyObj={
	...
	open : function(dataId,ftrIdn){
		...
		var url = CONTEXT_PATH + "/enviro/" + ftrIdn + "/modifyPageReant.do";
		var id = windowObj.createWindow(that.CLASS_NAME, that.TITLE, url, null, windowOptions, function() {
			that.init();
			that.initUi();
			that.bindEvents();
	...
	init : function() {
		...
		$("form", that.selector).ajaxForm({
			...
			success : function(result) {
				if(result.state >= 1) {
					that.close();
					$.messager.alert(that.TITLE, "제거작업일지가 변경되었습니다.");
				}
				else {
					$.messager.alert(that.TITLE, "제거작업일지 편집에 실패하였습니다.");
				}
			}
		});
	},
	...
	initUi : function(){
		...
		$("#reantModify .easyui-textbox", that.selector).textbox();
		$("#reantModify .easyui-numberbox", that.selector).numberbox({min:0, precision:2});
		$("#reantModify .easyui-combobox", that.selector).combobox();
		$("#reantModify .easyui-datebox", that.selector).datebox();
		$("#reantModify .easyui-numberspinner", that.selector).numberspinner();
			var data = dongObj.getData();
		...
		var pnu = $("input[name=pnu]", that.selector).val();	// 기점
		if (pnu && pnu.length > 0) {
	...
	modify : function() {
		...
		var url = CONTEXT_PATH + "/enviro/" + that.ftrIdn + "/modifyReant.do";
	...
	close : function() {
		...
		windowObj.removeWindow(that.selector);
		reantObj.registerObj.open(that.ftrIdn);
		that.dataId = null;
		that.ftrIdn = null;
		that.selector = null;
	}	
...
reantObj.photoAddObj = {
	...
	open : function(parent, dataId, ftrIdn) {
		...
		var url = CONTEXT_PATH + "/enviro/" + dataId + "/" + ftrIdn + "/addReantPhoto.do";
		var id = windowObj.createWindow(that.CLASS_NAME, that.TITLE, url, null, windowOptions, function() {
			that.init();
			that.initUi();
			that.bindEvents();
	...
	insert : function() {
		...
		//작업전 / 작업후
		$.ajax({
			url : CONTEXT_PATH + "/enviro/" + that.ftrIdn + "/searchRePhotoList.do",
			async : false,
			type : "GET",
			success : function(data) {
				var rows = null;
				...
				if (data.result.length>0){
					for(var i=0; i<data.result.length; i++){
						...
						//2개이상이면  페일
							...
							$.messager.alert(that.TITLE, "작업 전 후 사진은 각 각 한장씩 등록가능 합니다.");
							return false;
						}else{
							//1개일경우
							//검색결과와 같으면 페일
...
reantObj.photoObj={
	...
	*/
	...
	open : function(parent, imageNo) {
		...
		var url = CONTEXT_PATH + "/enviro/" + imageNo + "/modifyReantPhoto.do";
		var id = windowObj.createWindow(that.CLASS_NAME, that.TITLE, url, null, windowOptions, function() {
			that.init();
			that.initUi();
			that.bindEvents();
